    ...
    <section class="breadcrumbs">
      <div class="container">
        ...
        <div class="d-flex justify-content-between align-items-center">
          <h2>Pembelian Yang Sedang Berlangsung</h2>
          <ol>
            <li><a href="home-anggota.php">Home</a></li>
            <li>Sedang Berlangsung</li>
          </ol>
        </div>
        ...
        <div class="table-responsive mt-2">
                <table class="table table-bordered" id="bootstrap-data-table" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        <?php
                        $no=0;
                        $query_data_pembelian = mysqli_query($conn, "select * from merchandise_pembelian, merchandise where merchandise_pembelian.id_merchandise = merchandise.id_merchandise and merchandise_pembelian.id_anggota = '$data_user[id_anggota]' and status = 'Menunggu konfirmasi admin' order by merchandise_pembelian.tgl_beli");
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="text-center">
                                    <a href="anggota-pembayaran-merch.php?id=<?php echo $data_pembelian['id_pembelian_merch'] ?>" class="btn btn-primary btn-icon-split">
                        ...
                        <?php
                        $no=0;
                        $query_data_pembelian = mysqli_query($conn, "select * from merchandise_pembelian, merchandise where merchandise_pembelian.id_merchandise = merchandise.id_merchandise and merchandise_pembelian.id_anggota = '$data_user[id_anggota]' AND status = 'Belum dibayar' and datediff('$tgl', tgl_update) <= '$data_pembayaran[tenggat_waktu]' order by merchandise_pembelian.tgl_beli");
                        while ($data_pembelian = mysqli_fetch_assoc($query_data_pembelian)) {
                            $no++;
                            $kadaluarsa = date('Y-m-d', strtotime('+ '.$tenggat.' days', strtotime($data_pembelian['tgl_update'])));
                        ?>
